...
M.wrap_with_abbreviation = function()
    local request_abbreviation = function(to_replace, callback)
        local abbreviation = vim.fn.input("Abbreviation: ")
        if abbreviation == "" then return end
        local opts = { text = utils.trim(to_replace) }
        vim.lsp.buf_request(
            0,
            "emmet/expandAbbreviation",
            { abbreviation = abbreviation, language = vim.bo.filetype, options = opts },
    ...
    local for_visual_mode = function()
        ...
        local current_lines = vim.api.nvim_buf_get_text(0, start_mark[1]-1, start_mark[2], end_mark[1]-1, end_mark[2]+1, {})
        request_abbreviation(table.concat(current_lines, "\n"), function(result)
    ...
    local for_normal_mode = function()
        ...
        request_abbreviation(current_line, function(result)
            local lines = vim.split(result, "\n")
